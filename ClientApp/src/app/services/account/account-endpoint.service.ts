...
export class AccountEndpoint extends EndpointFactory {
  ...
  private readonly _userEmployeeData: string = "/api/account/getUserEmployeeData";
  ...
  get userEmployeeDataUrl() { return this.configurations.baseUrl + this._userEmployeeData; }
    ...
    getUsersEndpoint<T>(page?: number, pageSize?: number, name?: string): Observable<T> {
        let endpointUrl = `${this.usersUrl}/${page}/${pageSize}/${name}`;
        return this.http.get<T>(endpointUrl, this.getRequestHeaders())
            .catch(error => {
  ...
  getPatchUpdateUserEndpoint<T>(valueOrPatch: any, opOrUserId?: string, path?: string, from?: any, userId?: string): Observable<T> {
    ...
    getRolesEndpoint<T>(page?: number, pageSize?: number, name?: string): Observable<T> {
        let endpointUrl = `${this.rolesUrl}/${page}/${pageSize}/${name}`;
        return this.http.get<T>(endpointUrl, this.getRequestHeaders())
            .catch(error => {
                return this.handleError(error, () => this.getRolesEndpoint(page, pageSize, name));
            });
    }
  ...
  getUserEmployeeEndpoint(id: string) {
    let endpointUrl = `${this.userEmployeeDataUrl}/${id}`;
    return this.http.get(endpointUrl, this.getRequestHeaders()).catch(error => { return this.handleError(error, () => this.getUserEmployeeEndpoint(id)) });
  }
 }
