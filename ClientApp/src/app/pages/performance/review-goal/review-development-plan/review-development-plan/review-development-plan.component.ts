...
@Component({
  selector: 'review-development-plan',
  templateUrl: './review-development-plan.component.html',
  styleUrls: ['./review-development-plan.component.css']
})
export class ReviewDevelopmentPlanComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private reviewGoalService: ReviewGoalService, private route: ActivatedRoute, private alertService: AlertService) { }
  ...
  getDevelopmentGoalDetail() {
    this.reviewGoalService.getDevelopmentPlanList(this.empId).subscribe(result => this.onSuccessfulDataLoad(result), error => this.onDataLoadFailed(error));
  }
  ...
  saveDevelopmentPlan() {
    ...
    this.reviewGoalService.saveDevelopmentPlan(this.empId, this.developmentPlan, 'save').subscribe(result => this.onSaveSuccessfulDataLoad(result, 'save'), error => this.onSaveSubmitDataLoadFailed(error));
  ...
  submitDevelopmentPlan() {
    ...
    this.developmentPlan.employeeCareerDevList = this.developmentForm.controls.employeeCareerDevList.value;
    this.developmentPlan.managerCareerDevList = this.developmentForm.controls.managerCareerDevList.value;
    this.reviewGoalService.saveDevelopmentPlan(this.empId, this.developmentPlan, 'submit').subscribe(result => this.onSaveSuccessfulDataLoad(result, 'submit'), error => this.onSaveSubmitDataLoadFailed(error));
  }
